...
class ObjFinder:
    def __init__(self):
        ...
        rospy.init_node('color_camera_10_frames')
        self.bridge = CvBridge()
        ...
        for dt in depth_topics:
            try:
                ...
                pass
        self.objects_list = ['candle', 'duck', 'diya']
        self.Box_list = ['Red', 'Pink', 'Green']
        self.items = {"Red": [0.2, 0.3, math.pi/2], 
         "Pink": [0.6, 0.3, math.pi/2],
         "Green": [0.9, 0.3, math.pi/2], 
         "candle": [0.45, -0.4, -math.pi/2],
         "duck": [0.8, -0.4, -math.pi/2], 
         "diya": [1.2, -0.4, -math.pi/2]}
    ...
    def approach_obj(self):
        ...
        # Now includes camera tilt to keep object centered both horizontally and vertically.
        ...
        kp_tilt = 0.002     # tilt gain for vertical centering
        pan_deadzone = 20   # pixels: acceptable horizontal error
        tilt_deadzone = 20  # pixels: acceptable vertical error
        distance_threshold = 0.35  # meters: stop when closer than this (if depth available)
        ...
        while not rospy.is_shutdown():
            ...
            # Pan control: horizontal steering (robot yaw)
            pan_error = error_x
            ang = -kp_ang * pan_error

            # Tilt control: vertical camera adjustment
            tilt_error = error_y
            tilt_adjust = np.clip(self.current_tilt_cmd - kp_tilt * tilt_error, 0.0, 1.0)
            ...
            # First priority: adjust camera to face object directly
            pan_centered = abs(pan_error) <= pan_deadzone
            tilt_centered = abs(tilt_error) <= tilt_deadzone
            ...
            try:
                ...
                rospy.logwarn(f"Camera tilt failed during approach: {e}")

            # Move forward only if camera is reasonably centered on target
            if pan_centered and tilt_centered:
                # Camera faces object → move forward with steering correction
                vel = Twist()
                vel.linear.x = 0.15
                vel.angular.z = -0.002 * pan_error  # small steering correction
                self.cmd_pub.publish(vel)
                rospy.loginfo(f"[APPROACH] Moving forward: area={area:.0f}, pan_err={pan_error:.1f}, tilt_err={tilt_error:.1f}, tilt_cmd={tilt_adjust:.2f}")
            else:
                # Camera not centered yet → adjust without moving forward
                vel = Twist()
                vel.linear.x = 0.0  # no forward motion while centering camera
                vel.angular.z = -0.002 * pan_error  # still apply light steering
                self.cmd_pub.publish(vel)
                rospy.loginfo(f"[CENTERING] pan_err={pan_error:.1f} (dead={pan_deadzone}), tilt_err={tilt_error:.1f} (dead={tilt_deadzone}), tilt_cmd={tilt_adjust:.2f}")
