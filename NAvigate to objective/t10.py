...
#adding realsense aso that i dont have to write the cmd
import pyrealsense2 as rs
from ultralytics import YOLO
# Import preloaded YOLO model from the Diwali script (uses BEST_DUCKS.pt)
import time
import math
...
class ObjFinder:
    ...
    def scan_camera2(self):
        """
        Sweeps the camera to find a duck using YOLO model, and centers the camera on the detection
        by adjusting pan/tilt based on the bounding box centroid's pixel location in the image.
        """
        tilt_angles = np.linspace(0, 1, 5)
        pan_angles = np.linspace(-1.04, 1.04, 7)

        rospy.loginfo("Scanning for duck using YOLO...")
        ...
        while not self.Obj_found and not rospy.is_shutdown():
            ...
            for tilt in tilt_angles:
                try:
                    self.robot.camera.tilt(tilt)
                    self.current_tilt_cmd = float(tilt)
                except Exception as e:
                    rospy.logwarn(f"Camera tilt failed in scan_camera2: {e}")
                ...
                for pan in pan_angles:
                    ...
                    # Look for 'duck' detections
                    for box in results[0].boxes:
                        class_id = int(box.cls[0])
                        class_name = self.model.names[class_id]
                        
                        if class_name.lower() == 'duck':
                            # Found a duck! Get bounding box and centroid
                            ...
                            # Map pixel error to pan/tilt adjustment
                            ...
                            rospy.loginfo(f"Duck detected and centered: pan={pan_adjust:.2f}, tilt={tilt_adjust:.2f}")

                            # Visual feedback
                            cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 2)
                            cv2.circle(img, (cx, cy), 5, (255, 0, 0), -1)
                            cv2.imshow("Duck Detection", img)
                            cv2.waitKey(1)

                            rospy.loginfo("Duck detected and centered!")
                            self.Obj_found = True
                            return

        rospy.loginfo("Finished scanning for duck.")
    ...
    def locate_and_approach_duck(self):
        """Locate 'duck' detections using the YOLO model and drive toward the largest one.
        Stops when depth indicates the robot is within `distance_threshold` meters.
        """
        rospy.loginfo("Locating and approaching ducks using YOLO model...")
        kp_ang = 0.003
        lin_speed = 0.12
        ...
        while not rospy.is_shutdown():
            ...
            if len(results) == 0 or len(results[0].boxes) == 0:
                rospy.loginfo("No detections — stopping.")
                self.cmd_pub.publish(Twist())
                self.rate.sleep()
                continue

            # Collect 'duck' detections and pick the largest
            ducks = []
            for box in results[0].boxes:
                class_id = int(box.cls[0])
                class_name = self.model.names[class_id]
                if class_name.lower() == 'duck':
                    x1, y1, x2, y2 = [int(x) for x in box.xyxy[0].tolist()]
                    area = (x2 - x1) * (y2 - y1)
                    ducks.append((area, x1, y1, x2, y2))

            if not ducks:
                rospy.loginfo("No duck class detected.")
                self.cmd_pub.publish(Twist())
                self.rate.sleep()
                continue

            ducks.sort(reverse=True)
            _, x1, y1, x2, y2 = ducks[0]
            cx = (x1 + x2) // 2
            cy = (y1 + y2) // 2
            ...
            cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 2)
            cv2.circle(img, (cx, cy), 5, (255, 0, 0), -1)
            cv2.imshow("YOLO Duck", img)
            cv2.waitKey(1)

            # Check depth around centroid (if available)
            reached = False
            ...
            if self.depth_image is not None:
                ...
                if np.isfinite(mean_depth):
                    rospy.loginfo(f"Mean depth at duck centroid: {mean_depth:.3f} m")
                    if mean_depth <= distance_threshold:
                        rospy.loginfo("Reached duck by depth threshold.")
                        reached = True
            ...
            # Steering control based on horizontal pixel error
            error_x = cx - img.shape[1] // 2
            ang = -kp_ang * error_x
            vel = Twist()
            ...
            vel.angular.z = ang
            self.cmd_pub.publish(vel)
            rospy.loginfo(f"Approaching duck: area={(x2-x1)*(y2-y1)}, cx={cx}, ang={ang:.3f}, depth={mean_depth:.3f}")
    ...
    def run(self):
        # First, scan camera to detect and center on the duck
        self.scan_camera2()

        if self.Obj_found:
            rospy.loginfo("Duck found! Approaching...")
            self.locate_and_approach_duck()
        else:
            rospy.loginfo("Duck not found during initial scan.")
